...
let MemoryDatabaseFromHell = require('./MemoryDatabaseFromHell');

describe('The memory database', () => {
  let database;

  beforeEach(() => {
    database = new MemoryDatabaseFromHell();
  });
  ...
  it('should find all documents based on deep criteria using a dot notation string', () => {
    let documents = [
      {id: '1', property: {deep: 'right'}},
      {id: '2', property: {deep: 'wrong'}},
      {id: '3', property: {deep: 'right'}},
  ...
  it('should create document if upsert is specified on update', () => {
    database.collections.collection = [];

    let update = database.updateFirst('collection', {id: '1'}, {$set: {name: 'name'}}, {upsert: true});

    return update.then(() => {
      database.collections.collection.should.deep.equal([{id: '1', name: 'name'}]);
    });
  });

  it('wont create document if upsert is not specified on update', () => {
    database.collections.collection = [];

    let update = database.updateFirst('collection', {id: '1'}, {$set: {name: 'name'}});
